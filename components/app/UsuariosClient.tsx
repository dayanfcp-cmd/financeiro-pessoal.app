      ...
      <div className="bg-white rounded-[20px] shadow-[0_2px_4px_rgba(76,60,150,.05),0_14px_34px_rgba(76,60,150,.09)] px-3">
        {membros.map((m) => (
          <div key={m.id} className="flex items-center gap-3 py-3.5 border-b border-[#E9E7F5] last:border-0">
            <span className="w-10 h-10 rounded-full grid place-items-center text-white font-extrabold text-[14px] flex-none" style={{ background: m.cor || "#6C4BF4" }}>{m.nome.charAt(0).toUpperCase()}</span>
            <div className="flex-1 min-w-0">
              ...
              <div className="flex gap-1.5 flex-wrap mt-1.5">
                {m.modulos.map((mod) => <span key={mod} className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${MOD_STYLE[mod] || "bg-gray-100 text-gray-600"}`}>{MODULOS_DISPONIVEIS.find((d) => d.id === mod)?.label || mod}</span>)}
              </div>
            </div>
            {souDono && <button onClick={() => setSheet({ editando: m })} className="text-[#6E7091] p-2"><Icon name="chev" className="w-4 h-4" /></button>}
          </div>
        ))}
      </div>
...
function SheetUsuario({ alvo, souEuMesmo, onFechar, onSalvo }: { alvo: Profile | null; souEuMesmo: boolean; onFechar: () => void; onSalvo: () => void; }) {
    ...
    try {
      if (editando && alvo) {
        ...
        const res = await fetch("/api/usuarios", {
          ...
          body: JSON.stringify({ email: email.trim(), senha, nome: nome.trim(), username: username.trim(), modulos }),
        });
  ...
  async function remover() {
    ...
    try {
      const res = await fetch("/api/usuarios", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: alvo.id }) });
      const data = await res.json();
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end justify-center">
      ...
      <div className="relative bg-white w-full max-w-[460px] rounded-t-[24px] p-5 pb-[calc(24px+env(safe-area-inset-bottom))] max-h-[88vh] overflow-y-auto">
        ...
        {!editando && <>
          <label className="block mb-3.5">
            <span className="block text-[12px] font-bold text-[#6E7091] mb-1.5">Nome de usuário</span>
            <input value={username} onChange={(e) => setUsername(e.target.value.toLowerCase().replace(/\s/g, ""))} autoCapitalize="none" autoCorrect="off" placeholder="ex.: anna" className="w-full border border-[#E9E7F5] rounded-xl px-3.5 py-2.5 text-[14.5px] outline-none" />
            <span className="block text-[11px] text-[#6E7091] mt-1.5">Será usado para entrar no app. Pode ser diferente do nome.</span>
          </label>
          <label className="block mb-3.5">
        ...
        <div className="mb-2">
          ...
          {MODULOS_DISPONIVEIS.map((m) => <div key={m.id} className="flex items-center justify-between py-2.5 border-b border-[#E9E7F5] last:border-0"><span className="text-[14px] font-semibold">{m.label}</span><button onClick={() => toggleModulo(m.id)} disabled={souEuMesmo && m.id === "usuarios"} className={`w-[42px] h-[25px] rounded-full relative transition disabled:opacity-40 ${modulos.includes(m.id) ? "bg-[#CF8A1C]" : "bg-[#E9E7F5]"}`}><span className={`absolute top-[2.5px] w-5 h-5 rounded-full bg-white shadow transition-all ${modulos.includes(m.id) ? "left-[19px]" : "left-[3px]"}`} /></button></div>)}
        </div>
