...
export function PainelAtividades({ meuId, householdId, membros }: { meuId: string; householdId: string; membros: Profile[] }) {
  ...
  async function carregar() {
    ...
    setAdiantamentos((o.data ?? []) as ActivityOccurrenceOverride[]);
    setCarregando(false);
  }

  useEffect(() => { carregar(); }, [householdId, periodo]);

  const datas = useMemo(() => Array.from({ length: periodoAtual.dias }, (_, i) => addDays(inicio, i)), [inicio.getTime(), periodoAtual.dias]);
  ...
  const porPessoa = useMemo(() => membros.map(p => {
    const minhas = ocorrencias.filter(o => o.activity.responsavel === p.id);
    let realizadas = 0;
    const realizadasDetalhes: { activity: Activity; data: string; completion: ActivityCompletion }[] = [];
    for (const o of minhas) {
      const c = conclusaoPorOcorrencia.get(`${o.activity.id}|${o.data}`);
      const ok = o.activity.tipo === "flex" ? c?.validacao_resultado === true : !!c;
      if (ok && c) { realizadas++; realizadasDetalhes.push({ activity: o.activity, data: o.data, completion: c }); }
    }
...
function ResumoDestaque({ titulo, nome, avatar, total, realizadas }: { titulo: string; nome: string; avatar?: string | null; total: number; realizadas: number }) { const pct = total ? Math.round(realizadas / total * 100) : 0; return <div className="bg-white rounded-[20px] p-4 shadow-[0_2px_4px_rgba(76,60,150,.05),0_14px_34px_rgba(76,60,150,.09)]"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full overflow-hidden bg-[#ECE8FF] grid place-items-center text-[#6C4BF4] font-extrabold">{avatar ? <img src={avatar} alt={nome} className="w-full h-full object-cover"/> : nome.charAt(0).toUpperCase()}</div><div><div className="text-[11px] font-extrabold text-[#6E7091]">{titulo}</div><div className="text-[15px] font-black mt-1">{nome}</div></div></div><div className="text-[25px] font-black text-[#12A87A] mt-2">{realizadas}<span className="text-[15px] text-[#9A9BB0]">/{total}</span></div><div className="text-[10px] text-[#6E7091]">realizadas / total</div><div className="mt-2 h-1.5 rounded-full bg-[#F0EFF6] overflow-hidden"><div className="h-full rounded-full bg-[#12A87A]" style={{ width: `${pct}%` }}/></div></div> }
